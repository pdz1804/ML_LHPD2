\section{Project Workflow and Implementation}
...
\subsection{Implementation Quality and Code Efficiency}
...
In our sentiment analysis problem, we defined and implemented two major use case pipelines:

\begin{enumerate}
    \item \textbf{Raw Feature Modeling Pipeline} – In this approach, we focused on building models directly on engineered features:
    \begin{itemize}
        \item We experimented with various feature representations including \textbf{Count Vectorizer}, \textbf{TF-IDF}, \textbf{GloVe}, and \textbf{Word2Vec} embeddings to numerically encode text.
        \item These features were used as input to traditional models such as \textbf{Logistic Regression}, \textbf{Random Forest}, \textbf{XGBoost}, and \textbf{SVM}. For SVM specifically, we explored \textbf{custom kernel functions for text}, \textbf{memory-efficient sparse representations}, and applied \textbf{soft-margin optimization} through hyperparameter tuning.
        \item \textbf{CRF} was utilized for its sequence labeling capability. By combining it with a BiLSTM encoder, we implemented a \textbf{BiLSTM-CRF model} to capture contextual relationships and sequential dependencies in sentiment-bearing words.
        \item \textbf{Ensemble methods} such as \textbf{Voting} and \textbf{Stacking} were implemented to combine multiple base learners, leveraging their complementary strengths for better generalization.
    \end{itemize}

    \item \textbf{Dimensionality Reduction Pipeline} – This use case introduces a dimensionality reduction step into the feature modeling pipeline:
    \begin{itemize}
        \item After building the same high-dimensional representations as above, we applied \textbf{Principal Component Analysis (PCA)} to reduce the feature space while retaining most of the variance.
        \item The reduced vectors were then used for training and inference using the same set of models.
        \item This approach provides advantages such as \textbf{reduced computational cost}, \textbf{lower memory consumption}, and \textbf{potential improvements in model generalization} due to elimination of redundant and noisy features.
    \end{itemize}
\end{enumerate}

\subsection{Data Preprocessing and Model Tuning}
...
\subsection{Performance Analysis and Model Evaluation}
